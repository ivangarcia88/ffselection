...
// RapidMIC recieves index to pair and assigned thread id
double MaxEquipartedGridScore(int pair, long t)
{
	...
	for (cols = 2; cols < g_max1DPartition; cols++)
	{
		...
		{
			// If the grid resolution is viable, compute RapidMIC
			if (rows * cols < nAlpha)
			{
...
void *RapidMICThread(void *t)
{
	...
	// Set auxiliar variables for the RapidMIC computation
	bool usePearson;
	double minPearson;
	double maxPearson;
	double mic;
	...
	for (i = start; i < end; i++)
	{
		// Use Pearson as a RapidMIC approximation if needed
		if (usePearson)
		{
			...
			// If the data is neither too noisy nor to notably linear, compute RapidMIC as usual
			else
			{
				varX = &*g_inputMatrix[g_results[i].var1];
				varY = &*g_inputMatrix[g_results[i].var2];
		...
		g_results[i].rapidmic = mic;
	}
	pthread_exit(NULL);
}


bool PerformRapidMIC(double **inputMatrix, int totalRows, int totalCols, mine_result *results, long totalResults, int numThreads, config params)
{
	...
	for (t = 0; t < numThreads; t++)
	{
		rc = pthread_create(&thread[t], &attr, RapidMICThread, (void*)t);
		if (rc)
