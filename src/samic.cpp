...
// SAmic recieves index to pair and assigned thread id
double SAmic(int pair, long t)
{
...
void *SAmicThread(void *t)
{
	...
	// Set auxiliar variables for the SAMIC computation
	bool useParallelMIC;
	double minParallelMIC;
	double maxParallelMIC;
	double mic;
	...
	// Set values for useParallelMIC, minParallelMIC and maxParallelMIC
	useParallelMIC = g_params.useParallelMIC && g_params.performParallelMIC;
	minParallelMIC = g_params.minParallelMIC;
	maxParallelMIC = g_params.maxParallelMIC;

	// Iterate over the results array on a restricted block of results
	for (i = start; i < end; i++)
	{
		// Use ParallelMIC as SAMIC approximation if needed
		if(useParallelMIC)
		{
			// If ParallelMIC indicates data is too noisy or a good fit then avoid computing SAMIC
			if (g_results[i].parallelmic <= minParallelMIC || g_results[i].parallelmic >= maxParallelMIC)
				mic = g_results[i].parallelmic;
			else
				mic = MAX(g_results[i].parallelmic, SAmic(i, (long) t));
		}

		// Compute SAMIC if needed
		else
			mic = SAmic(i, (long) t);

		// Store resulting MIC metric to the corresponding result
		g_results[i].samic = mic;
	}
	pthread_exit(NULL);
}


bool PerformSAMIC(double **inputMatrix, int totalRows, int totalCols, mine_result *results, long totalResults, int numThreads, config params)
{
	...
	for (t = 0; t < numThreads; t++)
	{
		rc = pthread_create(&thread[t], &attr, SAmicThread, (void*)t);
		if (rc)
